...
void readFile(string path, int& nbPoints, int& sizePoint, double*& points, int*& labels, map<string, int>& labelToIdx, map<int, string>& idxToLabel){
    // ifs = pra ler arquivo
    ifstream file(path);
    if (!file.is_open()) {
...
// se é chamada por um kernel deve ser "__device__"
...
__global__ void KERNEL_compute_distances(double *pt, int K, int nbPoints, int sizePoint, double* kDists, int* kLabels, double* points, int* labels){
    ...
    // tem mais threads na GPU do que quantidades de pontos
    ...
    //para cada thread os K melhores q ela encontrou
}
...
int applyKnn(double* pt, int K, int nbPoints, int sizePoint, double* device_points, int* device_labels, int nbThreadsPerBlock, int nbBlocks){
    ...
    // alocar na CPU o vetor inteiro
    // é bom colocar tudo num vetor pq n precisar ficartransferindo sempre p ordenar
...
int main(int argc, char *argv[])
{
    ...
    // host
    readFile(TRAIN_FILE, nbPoints, sizePoint, points, labels, labelToIdx, idxToLabel);
    ...
    readFile(TEST_FILE, nbPointsTest, sizePointTest, pointsTest, labelsTest, labelToIdx, idxToLabel);
    ...
    startMemo = clock();
    // mandando os pontos de treino da cpu p gpu
    auto [device_points, device_labels] = setPointsGpu(nbPoints, sizePoint, points, labels);
    endMemo = clock();

    /* --------------- Escolhendo um ponto para fazer inferência -------------- */
    // trocar 42 por var
    double *pt = &pointsTest[42 * sizePointTest];
    int label = labelsTest[42];
    ...
    // tempo pra inferência
    cout << "TIME: " << time_in_ms(start, end) << "\n";
    // tempo pra transferir da cpu p gpu
    cout << "TIME FOR MEMORY: "<< time_in_ms(startMemo, endMemo) << "\n";
    //cout << time_in_ms(start, end) + time_in_ms(startMemo, endMemo);

    freeHostMemory(points);
    freeHostMemory(labels);
    freeHostMemory(pointsTest);
    freeHostMemory(labelsTest);
